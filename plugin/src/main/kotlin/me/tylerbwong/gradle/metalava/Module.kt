...
internal sealed class Module {
  ...
  class Android(val extension: LibraryAndroidComponentsExtension) : Module() {
    /**
     * The map of available library variants to be passed into [compileClasspath] so as to filter
     * its output.
     *
     * This map is populated dynamically during the Android Gradle configuration phase, typically
     * from an `onVariants` callback when each [LibraryVariant] is registered. Callers should
     * therefore only rely on its contents after variant registration has completed.
     *
     * @see compileClasspath
     */
    ...
    override fun compileClasspath(project: Project, variant: String?): FileCollection {
      return javaModule.compileClasspath(project, variant) +
        (kotlinExtension.targets
          ...
          .filter {
            it.defaultSourceSet.name.contains(SourceSet.MAIN_SOURCE_SET_NAME, ignoreCase = true)
          }
      ...
      return project.files(
        javaModule.sourceSets(project, variant) +
          kotlinExtension.sourceSets
            .filterNot {
              it.name.lowercase(Locale.getDefault()).contains(SourceSet.TEST_SOURCE_SET_NAME)
            }
  ...
  class Java(private val extension: JavaPluginExtension) : Module() {
    ...
    override fun compileClasspath(project: Project, variant: String?): FileCollection {
      return extension.sourceSets
        ...
        .reduceOrNull(FileCollection::plus)
        ?.filter { it.exists() && it.checkDirectory(listOf(".jar", ".class")) } ?: project.files()
    }
  ...
  class Composite(private val modules: List<Module>) : Module() {
    ...
    override fun sourceSets(project: Project, variant: String?): FileCollection {
      return modules.map { it.sourceSets(project, variant) }.reduceOrNull(FileCollection::plus)
        ?: project.files()
    }

    internal inline fun <reified T : Module> extract(): T? = modules.firstOrNull { it is T } as? T
  }
